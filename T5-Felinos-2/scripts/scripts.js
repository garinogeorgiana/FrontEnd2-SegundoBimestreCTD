...
/* capturamos el boton y aplicamos evento para cambiar entre tema claro y oscuro */
...
listadoFelinos.forEach((felino) => {
    // creo un div y le damos la clase "item" 
    let div = document.createElement("div")
    div.classList.add("item")

    // creamos la etiqueta img y seteamos el atributo src conla imagen 
    let img = document.createElement("img")
    img.setAttribute("src",felino.imgUrl)

    // creamos el titulo con su node de texto 
    let h2 = document.createElement("h2")
    let textoH2 = document.createTextNode(felino.title)
    h2.appendChild(textoH2)

    // creamos el parrafo con su textNode 
    let p = document.createElement("p")
    let textoP = document.createTextNode(felino.description)
